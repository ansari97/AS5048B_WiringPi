...
double AS5048B::angle(int unit, bool newVal){
    ...
    // in case LSB:MSB
    value_MSB = value_16bit & 0xFF; // MSB is already in position, just cut of the unwanted part
    value_LSB = value_16bit >> 8; // shift LSB to the right position

    // then the normal bitbanging to get the 14bit value (according the datasheet of the sensor)
    value_14bit = ((uint16_t) value_MSB) << 6; // cast the 8 bit value to 16bit size and shift the MSB value 6 position to the left
    value_14bit += (value_LSB & 0x3f);  // mask/remove the top two bits of LSB and add the other to the previous result to get the final result

    raw_ang = (double) value_14bit; // cast the unsigned 16 bit value to dourble floatingpoint value
...
void AS5048B::doProgZero(void) {
  ...
  wiringPiI2CReadReg8(file, AS5048B_ANGLMSB_REG);
  usleep(10000);
